...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec  3 15:19:10 2023
...
# change the variance of group b
...
dimension = 5
inflate_list = range(10,10+5*3,3) 

if data_type == "cluster":
 varShift = [0, 0.05, 0.1, 0.2, 0.5, 1, 2, 4, 6]
else: 
    varShift = [0, 0.05, 0.1, 0.2, 0.5, 1, 1.5, 2, 3]
...
for e in range(len(seed)):
    ...
    for i in range(len(varShift)):
        ...
        #shift mean
        df.variance_shift("add_both-a", varShift[i])
...
matrix_plot.plot_line_final(axis_title, varShift, flag_rate_a, flag_rate_b, base_ratea, base_rateb, recall_a, recall_b, tpr_whole, fpr_a, fpr_b, fpr_whole, ppr_a, ppr_b, ppr_whole)
matrix_plot.auroc(auroc, varShift, axis_title)


bias_type = "variance inflation_add_max6"


# save.save(bias_type, data_type, method, varShift, 
