...
class AEModel():
    ...
    def fit(self, train_X: np.ndarray):
        ...
        for it in tqdm(range(self.epochs)):
            final_loss = 0.0 # Initialize loss for current epoch
            start_time = time.time()
            epoch_loss = 0.0 # Initialize epoch loss
            for idx in range(total_batches):
                batch_index, data = loader.get_next_batch(idx)
             #   print(data.shape)
                data = data.type(torch.FloatTensor).to("cuda") # Convert data to float tensor and move to the device
              #  print(data[0])
                output = self.AE(data) # Forward pass: compute the autoencoder output
              #  print(output)
